...
router.get("/details/:id", authMiddleWare, async (req, res, next) => {
    // Get Event Details
    // Consolidate endpoint for "/events/id/:id", "/events/attendees/:id", and "/services"
    const eventDetails = {};
    try {
        eventDetails.event = await events.findOne({ _id: req.params.id, org: org });
        const attendeeEvent = await events.findById(req.params.id);
        const attendeeIds = attendeeEvent.attendees.map((attendee) => attendee.toString());
        eventDetails.attendees = await clients.find({ _id: { $in: attendeeIds } });
        eventDetails.services = await services.find({ org: org }).sort({ updatedAt: -1 });
    } catch (err) {
        return next(err);
    }
...
router.put("/register", authMiddleWare, (req, res, next) => {
    events.find({ _id: req.query.event, attendees: req.query.client, org: org }, (error, data) => {
        if (error) {
            return next(error);
        } else {
            // only add attendee if not yet signed up
            if (!data.length) {
                events.findByIdAndUpdate(req.query.event, { $push: { attendees: req.query.client } }, (error, data) => {
                    if (error) {
                        return next(error);
                    } else {
...
router.put("/deregister", authMiddleWare, (req, res, next) => {
    events.findByIdAndUpdate(req.query.event, { $pull: { attendees: req.query.client } }, (error, data) => {
        if (error) {
            return next(error);
        } else {
            clients.findByIdAndUpdate(req.query.client, { $pull: { events: req.query.event } }, (error, data) => {
                if (error) {
                    return next(error);
                } else {
