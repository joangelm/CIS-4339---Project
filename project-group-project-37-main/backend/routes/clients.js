...
router.get("/id/:id", authMiddleWare, (req, res, next) => {
    clients.findOne({ _id: req.params.id, orgs: org }, (error, data) => {
        if (error) {
            ...
            // Before sending client data
            // Get client profile picture from uploads folder
            if (data.profilePicture) {
                data.profilePicture = req.protocol + "://" + req.get("host") + "/" + data.profilePicture;
            }
            res.json(data);
        }
    });
...
router.get("/details/:id", authMiddleWare, async (req, res, next) => {
    // Get Client by ID
    // Consolidate endpoint for "/client/id/:id", "/events/id/:id", and "/events/client/:id/not-registered"
    const clientDetails = {};
    try {
        clientDetails.client = await clients.findOne({ _id: req.params.id, orgs: org });
        if (clientDetails.client.profilePicture) {
            clientDetails.client.profilePicture = req.protocol + "://" + req.get("host") + "/" + clientDetails.client.profilePicture;
        }
        clientDetails.events = await events.find({ attendees: req.params.id, org: org });
        clientDetails.notRegistered = await events.find({
...
router.put("/update/:id", authMiddleWare, upload.single("profilePicture"), (req, res, next) => {
    const clientData = req.body;
    clientData.profilePicture = req.file ? `uploads/${req.file.filename}` : null;
    clients.findByIdAndUpdate(req.params.id, clientData, (error, data) => {
        if (error) {
            return next(error);
        } else {
        ...
        if (error) {
            ...
            events.find({ attendees: req.params.id, org: org }, (error, data) => {
                if (error) {
                    ...
                    else res.status(406).send("Client is signed up for events and can't be deleted.");
                }
            });
        }
    });
