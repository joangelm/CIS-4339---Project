...
<template>
    <main>
        <div>
            <!--Header-->
            <h1 class="font-bold text-4xl text-red-700 tracking-widest text-center mt-10">Event Details</h1>
        </div>
        <div class="px-10 py-20">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-x-6 gap-y-10">
                ...
                <!--Go back button-->
                <div class="flex justify-between mt-10 mr-20">
                    <button type="reset" class="border border-red-700 bg-white text-red-700 rounded" @click="this.$router.back()">Go back</button>
                </div>
            </div>
...
import { getEventDetails, updateEvent, deleteEvent } from "../api/api";
...
export default {
    ...
    async mounted() {
        ...
        try {
            // consolidated 3 requests into one request for eventdetails
            const response = await getEventDetails(this.$route.params.id);
            response.event.date = new Date(response.event.date).toISOString().substring(0, 10);
            this.event = response.event;
            this.clients = response.attendees;
            this.services = response.services;
            // this.inactiveServices = response.services.filter(item => item.status === "Inactive")
        } catch (error) {
    ...
    methods: {
        ...
        //function to consolidate 3 requests into one request for eventdetails
