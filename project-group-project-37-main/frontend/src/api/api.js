...
export async function loginUser(username, password) {
    try {
        ...
        if (error.response.status === 400) {
            let errorString = getErrorMessageFromResponseBody(error.response.data.message);
...
export const getEventDetails = async (id) => {
    try {
        // const [event, attendees, services] = await Promise.all([
        //     apiClient.get(`/events/id/${id}`),
        //     apiClient.get(`/events/attendees/${id}`),
        //     apiClient.get(`/services/`),
        // ]);
        const response = await apiClient.get(`/events/details/${id}`);
        return {
...
// API call to GET events for event a event is signed up
export const attendees = async (id) => {
    try {
        const response = await apiClient.get(`/events/client/${id}`);
        return response.data;
    } catch (error) {
...
// API Call to GET client details by ID
export const getClientDetails = async (id) => {
    try {
        // const [client, events, notRegistered] = await Promise.all([
        //     apiClient.get(`/clients/id/${id}`),
